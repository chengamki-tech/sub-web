...
        // =========================================
        backendOptions: [
          { value: "https://subscriptionlink.amki.site/sub?", label: "我的专属后端 (安全/极速)" }, // 您的地址
          { value: "http://127.0.0.1:25500/sub?", label: "本地默认后端 (仅供测试)" }, // 原来的地址
          { value: "https://api.wcc.best/sub?", label: "api.wcc.best (公共备用)" } // 添加一个公共备用地址
        ],
...
    currentBackend() {
      // 这里也需要更新 CONSTANTS.DEFAULT_BACKEND，但由于我无法修改 CONSTANTS，
      // 实际部署时，您需要在 CONSTANTS.js 中将 DEFAULT_BACKEND 设为您的域名
...
      // 获取后端版本时，默认使用第一个选项（您的专属后端）进行尝试
      const defaultBackend = this.options.backendOptions[0].value;
      this.backendVersion = await BackendService.getBackendVersion(this.$axios, defaultBackend);
    },
...
          "i",
          { style: "color: teal" },
          "各种订阅链接（短链接服务除外）生成纯前端实现，无隐私问题。默认使用您的专属后端，隐私担忧者可选择其他服务。"
        )
      });
    },
